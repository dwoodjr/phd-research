...
state = {
    ...
    "tds":         0.0,
    "tds_v":       0.0,
    "temperature": 25.0,   # default until first real read
    "piezo1":      0.0,
    "piezo2":      0.0,
    "motor_a":     0.0,
    "motor_b":     0.0,
...
async def task_osc():
    ...
    while True:
        if state["wifi_ok"] and state["osc"]:
            ...
            messages = [
                ("/sensors/turbidity",   state["turbidity"]),
                ("/sensors/tds",         state["tds"]),
                ("/sensors/temperature", state["temperature"]),
                ("/sensors/piezo1",      state["piezo1"]),
                ("/sensors/piezo2",      state["piezo2"]),
                ("/motors/a_speed",      state["motor_a"]),
                ("/motors/b_speed",      state["motor_b"]),
            ]
